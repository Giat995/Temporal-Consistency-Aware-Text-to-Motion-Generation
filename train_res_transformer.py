...
from options.train_option import TrainT2MOptionsRes
...
def load_vq_model():
    opt_path = pjoin(opt.vq_dir, 'opt.txt')
    ...
    vq_model = RVQVAE(vq_opt,
                ...
                vq_opt.frb,
                opt.device)
    ckpt = torch.load(pjoin(opt.vq_dir, 'net_best_fid.pth'),
...
if __name__ == '__main__':
    parser = TrainT2MOptionsRes()
    opt = parser.parse()
    fixseed(opt.seed)

    opt.out_dir = pjoin(opt.out_dir, opt.dataset_name)
    ...
    opt.vq_dir = pjoin(opt.vq_dir, opt.dataset_name, opt.vq_name)
    vq_model, vq_opt = load_vq_model()
    opt.meta_dir = vq_opt.meta_dir
    ...
    trainer.train(train_loader, val_loader, eval_val_loader, eval_wrapper=eval_wrapper, logger=logger, writer=writer, plot_eval=plot_t2m)