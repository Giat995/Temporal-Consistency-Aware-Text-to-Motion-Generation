...
def load_trans_model(model_opt, which_model):
    t2m_transformer = MaskTransformer(code_dim=model_opt.code_dim,
                                      ...
                                      opt=model_opt)
    ckpt = torch.load(pjoin(model_opt.out_dir, which_model),
                      map_location=opt.device)
...
def load_res_model(res_opt):
    ...
    ckpt = torch.load(pjoin(res_opt.out_dir, 'net_best_fid.tar'),
                      map_location=opt.device)
...
if __name__ == '__main__':
    ...
    model_opt_path = pjoin(opt.mtrans_dir, opt.dataset_name, opt.mtrans_name, 'opt.txt')
    model_opt = get_opt(model_opt_path, device=opt.device)

    vq_opt_path = pjoin(opt.vq_dir, opt.dataset_name, model_opt.vq_name, 'opt.txt')
    ...
    t2m_transformer = load_trans_model(model_opt, which_model='net_best_fid.tar')

    res_opt_path = pjoin(opt.rtrans_dir, opt.dataset_name, opt.rtrans_name, 'opt.txt')
    ...
    for i in range(repeat_time):
        with torch.no_grad():
            best_fid, best_div, Rprecision, best_matching, best_mm, logger, writer = \
                eval_t2m.evaluation_mask_transformer_test_plus_res(eval_val_loader, vq_model, res_model, t2m_transformer,
                                                                    i, eval_wrapper=eval_wrapper, logger=logger, writer=writer, 
