...
@torch.no_grad()
def evaluation_mask_transformer(out_dir, val_loader, trans, vq_model, logger, writer, ep, best_epoch, best_fid, best_div,
...
@torch.no_grad()
def evaluation_res_transformer(out_dir, val_loader, trans, vq_model, logger, writer, ep, best_epoch, best_fid, best_div,
                           best_top1, best_top2, best_top3, best_matching, eval_wrapper, plot_func,
...
@torch.no_grad()
def evaluation_mask_transformer_test_plus_res(val_loader, vq_model, res_model, trans, repeat_id, eval_wrapper, logger, writer, 
    ...
    msg = f"--> \t Eva. Repeat {repeat_id} :, FID. {fid:.4f}, " \
          ...
          f"multimodality. {multimodality:.4f}"
    logger.info(msg)
    return fid, diversity, R_precision, matching_score_pred, multimodality, logger, writer

@torch.no_grad()
def evaluation_mask_transformer_test_plus_res_frb(val_loader, vq_model, res_model, trans, frb, repeat_id, eval_wrapper, logger, writer, 
                                time_steps, cond_scale, temperature, topkr, gsample=True, force_mask=False,
                                              cal_mm=True, res_cond_scale=5):
    trans.eval()
    vq_model.eval()
