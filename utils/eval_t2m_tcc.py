...
@torch.no_grad()
def evaluation_vqvae(out_dir, val_loader, net, logger, writer, it, best_iter, best_fid, best_div, best_top1,
    ...
    for batch in val_loader:
        ...
        if net.tcc_flag:
            pred_pose_eval, loss_commit, perplexity, loss_tcc = net(motion)
        else:
    ...
    msg = "--> \t Eva. it %6d:, FID. %.4f, Diversity Real. %.4f, Diversity. %.4f, R_precision_real. (%.4f, %.4f, %.4f), R_precision. (%.4f, %.4f, %.4f), matching_score_real. %.4f, matching_score_pred. %.4f"%\
    ...
    if fid < best_fid:
        ...
        if draw: logger.info(msg)
        best_fid = fid
        best_iter = it
        if save:
            torch.save({'vq_model': net.state_dict()}, os.path.join(out_dir, 'net_best_fid.pth'))
    ...
    if matching_score_pred < best_matching:
        ...
        if save:
            torch.save({'vq_model': net.state_dict()}, os.path.join(out_dir, 'net_best_mm.pth'))

    if save:
         torch.save({'net': net.state_dict()}, os.path.join(out_dir, 'net_last.pth'))

    net.train()
    return best_iter, best_fid, best_div, best_top1, best_top2, best_top3, best_matching, logger, writer


def evaluation_frb(out_dir, val_loader, vq_net, fr_net, logger, writer, ep, best_epoch, best_fid, best_div, best_top1,
                     best_top2, best_top3, best_matching, eval_wrapper, save=True, draw=True):
    fr_net.eval()
    ...
    for batch in val_loader:
        ...
        if vq_net.tcc_flag:
            out, loss_commit, perplexity, loss_tcc = vq_net(motion)
        else:
            out, loss_commit, perplexity = vq_net(motion)

        
        pred_pose_eval = fr_net(out)
    ...
    if fid < best_fid:
        ...
        best_epoch = ep
        if save:
            torch.save({'pred_pose_eval': fr_net.state_dict()}, os.path.join(out_dir, 'net_best_fid.pth'))
    ...
    if matching_score_pred < best_matching:
        ...
        best_matching = matching_score_pred
        if save:
            torch.save({'pred_pose_eval': fr_net.state_dict()}, os.path.join(out_dir, 'net_best_mm.pth'))

    if save:
         torch.save({'net': fr_net.state_dict()}, os.path.join(out_dir, 'net_last.pth'))

    fr_net.train()
    return best_epoch, best_fid, best_div, best_top1, best_top2, best_top3, best_matching, logger, writer
